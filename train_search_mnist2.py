...
def eval_minibatch(x, y, generator_model, encoder_model, t_inf, r_inf, epoch, device
    ...
    if t_inf == 'unimodal' and r_inf == 'unimodal':
        ...
        # to prevent kl_z causing a nan value, where q(t,r) becomes zero
        if not torch.any(torch.isnan(q_t_r_temp)).item():
...
def main():
    ...
    if args.macro_arch == "down":
        encoder_model = SuperDownNetMNIST(rot_refinement, in_channels, z_dim, theta_prior, normal_prior_over_r)
    elif args.macro_arch == "unet":
        encoder_model = SuperUNetMNIST(rot_refinement, in_channels, z_dim, theta_prior, normal_prior_over_r)  
    ...
    experiment_description = '_'.join([str(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M'))
                                       , args.dataset, 'zDim', str(z_dim),'macro_arch', args.macro_arch, 'rot_refine'
                                       , str(args.rot_refine)])
    group_conv = 16
    #if group_conv > 0:
    ...
    early_stopping = EarlyStopping(patience=30, delta=1e-4, save_path=path_prefix, digits=digits)
